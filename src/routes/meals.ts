...
export async function mealsRoutes(app: FastifyInstance) {
	app.post(
		...
		async (req, rep) => {
			...
			if (!newMeal.success) {
				return rep.status(400).send(`Data invalid! ${JSON.stringify(newMeal.error.format())}`);
			}
	...
	app.put(
		...
		async (req, rep) => {
			...
			if (!updatedMeal.success) {
				return rep.status(400).send(`Data invalid! ${JSON.stringify(updatedMeal.error.format())}`);
			}
